...
- (void)viewDidLoad
{
   ...
   self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"back"] style:UIBarButtonItemStylePlain target:self action:@selector(onLeftItem)];
...
- (void)onLeftItem{
    [_conversation setReadAllMsg];//zll修改
  [self hiddenKeyBoard];//zllsig
  [self.navigationController popViewControllerAnimated:YES];//zllsig
  
//  AppDelegate *app = (AppDelegate *)[[UIApplication sharedApplication] delegate];
//  app.window.rootViewController = app.nav;
//  [app.nav setNavigationBarHidden:YES];
...
- (void)configWithUser:(IMAUser *)user
{
  ...
  if(_conversation.msgList&&_conversation.msgList.count>0){
     [_conversation.msgList removeAllObjects];//zll修改
  }
...
-(BOOL)checkSelfConversion:(NSString *)userId{
  if([_receiver.userId isEqualToString:userId]){
    return true;
  }else{
    return false;
  }
...
//        _isSendMsg = YES;
      ...
      NSArray *newaddMsgs = [_conversation sendMessage:msg completion:^(NSArray *imamsglist, BOOL succ, int code) {
            ...
            IMAMsg * msg1 = imamsglist.lastObject;
      
        [LiveCommonMethod isSendVideoInvite:msg1 user:_receiver];
...
- (void)showMsgs:(NSArray *)msgs
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      NSInteger roww = 0;
      if (_messageList) {
        roww = _messageList.count - 1;
      }
        NSIndexPath *index = [NSIndexPath indexPathForRow:roww inSection:0];
...
- (void)moreVideVideoAction
{
    // 隐藏键盘
    [self hiddenKeyBoard];
    [LiveCommonMethod sendVideoInvitation:_receiver.userId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    IMAMsg *msg = [_messageList objectAtIndex:indexPath.row];
//  if(msg.type==EIMAMSG_Text){
//     msg = [LiveCommonMethod filterVideoShowText:msg];
//  }
