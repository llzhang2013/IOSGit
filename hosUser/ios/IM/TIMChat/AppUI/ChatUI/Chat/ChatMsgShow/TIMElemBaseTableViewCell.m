...
- (void)configWith:(IMAMsg *)msg
{
    ...
    {
        user = [_msg getSender];
       [_icon sd_setImageWithURL:[NSURL URLWithString:user.icon] forState:UIControlStateNormal placeholderImage:kDefaultUserIcon];//zll修改
    }
...
- (NSArray *)showMenuItems
{
  ...
  [array addObject:copyItem];//zll修改
...
- (void)copyItem:(id)sender{
  UILabel *lab = (UILabel *)_elemContentRef;
  [UIPasteboard generalPasteboard].string = lab.text;
...
{
  BOOL can = ((action == @selector(deleteItem:)) || (action == @selector(resendItem:)) || (action == @selector(revokeItem:))||(action == @selector(copyItem:)));
  return can;//zll修改
}
