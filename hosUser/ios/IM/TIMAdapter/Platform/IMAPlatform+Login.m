...
- (void)login:(TIMLoginParam *)param succ:(TIMLoginSucc)succ fail:(TIMFail)fail
{
  ...
  NSString *userName = param.identifier;
  NSString *sign = param.userSig;
  [[ILiveLoginManager getInstance] iLiveLogin:userName sig:sign succ:^{
    NSLog(@"zllLive登录成功--%@",userName);
  ...
  //  [IMAPlatform setAutoLogin:YES];
  ...
  } failed:^(NSString *module, int code, NSString *msg) {
    
    DebugLog(@" zllLive登录失败TIMLogin Failed: code=%d err=%@", code, msg);
    if (fail)
    {
      fail(code, msg);
    }
    ...
    {
      [YNIMModel initShareMD].timLogin = msg;
...
- (void)offlineKicked:(TIMLoginParam *)param succ:(TIMLoginSucc)succ fail:(TIMFail)fail
{
  // 周传文修改与20170901
  DebugLog(@"踢下线通知1");
  [YNIMModel initShareMD].timLogin = @"timOnKick";
  // 退出
  [self logout:^{
  } fail:^(int code, NSString *msg) {
  }];
